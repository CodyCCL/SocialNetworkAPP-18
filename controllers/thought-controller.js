...
const thoughtCon = {
  ...
  async getSingleThought(req, res) {
    try {
      ...
      if (!dbThoughtData) {
        return res.status(404).json({ message: 'no thoughts associated with given id' });
      }
  ...
  async createThought(req, res) {
    try {
      ...
      if (!dbUserData) {
        return res.status(404).json({ message: 'no user thought with this id!' });
      }
